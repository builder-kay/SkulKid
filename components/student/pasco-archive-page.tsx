...
export function PascoArchivePage() {
  ...
  const visible = useMemo(() => {
    ...
    return quizzes.filter((quiz) =>
      ...
      && (!needle || `${quiz.title} ${quiz.description} ${quiz.className}`.toLowerCase().includes(needle))
    );
    ...
    <main className="mx-auto grid w-full max-w-7xl gap-5">
      ...
      <header className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-amber-500 via-orange-500 to-rose-600 p-6 text-white shadow-xl sm:p-8">
        ...
        <div className="relative flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
          <div><p className="flex items-center gap-2 text-xs font-black uppercase tracking-[.18em] text-amber-100"><BookMarked className="size-4" />Past questions</p><h1 className="mt-2 text-4xl font-black sm:text-5xl">PASCO</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-orange-50 sm:text-base">Review ended class quizzes, learn from every answer and practise again without affecting your scores.</p></div>
      ...
      <section className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid gap-3 md:grid-cols-[1fr_13rem_12rem]">
          ...
          <label><span className="sr-only">Filter by class</span><select className="min-h-12 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 font-bold" onChange={(event) => setClassId(event.target.value)} value={classId}><option value="all">All classes</option>{classes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      ...
      {loading ? <div className="grid min-h-64 place-items-center"><Loader2 className="size-7 animate-spin text-orange-600" /></div> : error ? <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 font-bold text-rose-900">{error}</div> : visible.length === 0 ? <div className="grid min-h-64 place-items-center rounded-[1.5rem] border border-dashed border-slate-300 bg-white p-8 text-center"><div><Sparkles className="mx-auto size-9 text-amber-500" /><h2 className="mt-3 text-xl font-black">{quizzes.length ? "No PASCO quizzes match" : "No past quizzes yet"}</h2><p className="mt-2 text-sm text-slate-600">{quizzes.length ? "Try another search or filter." : "Ended quizzes from your classes will appear here automatically."}</p></div></div> :
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{visible.map((quiz) => <article className="group flex flex-col overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:border-orange-300 hover:shadow-xl" key={quiz.id}><div className="h-2 bg-gradient-to-r from-amber-400 via-orange-500 to-rose-500" /><div className="flex flex-1 flex-col p-5"><div className="flex items-start justify-between gap-3"><span className="rounded-full bg-orange-50 px-3 py-1 text-[11px] font-black uppercase text-orange-800">{quiz.className} · Basic {quiz.gradeLevel}</span>{quiz.attempted ? <span className="inline-flex items-center gap-1 text-xs font-black text-emerald-700"><CheckCircle2 className="size-4" />Attempted</span> : <span className="text-xs font-black text-slate-500">Missed</span>}</div><h2 className="mt-4 text-xl font-black text-slate-950">{quiz.title}</h2><p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-600">{quiz.description || "Review the questions and practise this past challenge."}</p><div className="mt-4 grid grid-cols-3 gap-2 text-center"><CardStat label="Questions" value={quiz.questionCount} /><CardStat label="Best" value={quiz.bestScore == null ? "—" : `${quiz.bestScore}%`} /><CardStat label="Attempts" value={quiz.attemptCount} /></div>{quiz.offPlatformReward ? <p className="mt-4 rounded-xl bg-amber-50 p-3 text-xs font-bold text-amber-900"><Trophy className="mr-1 inline size-4" />Class reward: {quiz.offPlatformReward}</p> : null}<div className="mt-auto pt-5"><p className="flex items-center gap-1 text-xs font-bold text-slate-500"><Clock3 className="size-3.5" />Ended {new Date(quiz.endedAt).toLocaleString()}</p><Link className="mt-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 font-black text-white group-hover:bg-orange-600" href={`/pasco/${quiz.id}`}>Review and practise <ArrowRight className="size-4" /></Link></div></div></article>)}</section>}
    </main>
  </StudentShell>;
}
