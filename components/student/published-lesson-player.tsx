      ...
      <main className="mx-auto w-full max-w-6xl space-y-5">
        <section className="relative overflow-hidden rounded-[2.25rem] bg-gradient-to-br from-slate-950 via-violet-900 to-blue-700 p-6 text-white shadow-[0_30px_80px_rgba(49,46,129,.3)] sm:p-9 lg:p-11">
          ...
          <div className="relative grid gap-8 lg:grid-cols-[1fr_18rem] lg:items-end">
            <div><div className="flex flex-wrap gap-2"><span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-black uppercase tracking-wider ring-1 ring-white/20"><Sparkles className="size-4 text-amber-300" />Learning mission</span><span className="rounded-full bg-blue-300/20 px-3 py-1.5 text-xs font-black uppercase tracking-wider text-blue-100 ring-1 ring-blue-200/20">{lesson.difficulty}</span><span className="rounded-full bg-white/10 px-3 py-1.5 text-xs font-black text-blue-100 ring-1 ring-white/20">Lesson {lesson.lessonNumber ?? lesson.order}</span></div>{lesson.strand || lesson.subStrand || lesson.topic ? <p className="mt-5 text-sm font-bold text-cyan-200">{[lesson.strand, lesson.subStrand, lesson.topic].filter(Boolean).join(" → ")}</p> : null}<h1 className="mt-2 max-w-3xl text-4xl font-black leading-tight sm:text-5xl lg:text-6xl">{lesson.title}</h1><p className="mt-4 max-w-3xl text-base leading-8 text-blue-100 sm:text-lg">{lesson.description}</p>{lesson.contentStandard || lesson.indicator ? <div className="mt-5 grid max-w-3xl gap-2 rounded-2xl bg-white/10 p-4 text-sm ring-1 ring-white/15">{lesson.contentStandard ? <p><b className="text-cyan-200">Content standard:</b> {lesson.contentStandard}</p> : null}{lesson.indicator ? <p><b className="text-cyan-200">Indicator:</b> {lesson.indicator}</p> : null}</div> : null}<div className="mt-6 flex gap-3 rounded-2xl bg-white/10 p-4 ring-1 ring-white/15"><Target className="mt-1 size-5 shrink-0 text-amber-300" /><div><p className="text-xs font-black uppercase tracking-wider text-blue-200">Your mission</p><p className="mt-1 leading-7 text-white">{lesson.objective}</p></div></div></div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <Link className="inline-flex min-h-11 items-center gap-2 rounded-xl px-4 font-black text-slate-700 hover:bg-slate-50" href="/dashboard">Home</Link>
        </div>
      </main>
    </StudentShell>
  );
...
function QuizModal({ open, title, questions, passingScore, masteryScore, onClose, onClaim }: { open: boolean; title: string; questions: LessonBlock[]; passingScore: number; masteryScore: number; onClose: () => void; onClaim: (results: QuizAnswerResult[]) => void }) {
  ...
  const submit = () => {
    ...
    setGrade({ score, passed, wrong });
    setQuestionIndex(0);
    if (wrong.length < questions.length) dispatchSuccessMoment("correct", `quiz-pass-${Date.now()}`);
  };
  const retake = () => {
    setChoices({});
    setGrade(null);
