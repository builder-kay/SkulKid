...
import { useLeaderboard, type LeaderboardEntry } from "@/lib/student/leaderboard";

export function LeaderboardPage() {
  const { entries: ranked, currentUser, totalLearners, loading, error } = useLeaderboard();
  ...
  return <StudentShell activeItem="leaderboard"><main className="mx-auto grid w-full max-w-7xl gap-6">
    ...
    <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[var(--shadow-card)]" aria-labelledby="ranking-heading"><div className="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><div><h2 id="ranking-heading" className="text-2xl font-black">All learner rankings</h2><p className="mt-1 text-sm text-text-secondary">Ranked from first to last by total XP, then stars.</p></div><span className="inline-flex items-center gap-2 self-start rounded-full bg-blue-50 px-3 py-2 text-sm font-black text-blue-800"><Shield className="size-4" />Live standings</span></div><div className="divide-y divide-slate-100">{loading ? <p className="p-8 text-center font-bold text-text-secondary">Loading live standings…</p> : error ? <p className="p-8 text-center font-bold text-rose-700">{error}</p> : ranked.length ? ranked.map((learner) => <RankingRow learner={learner} current={learner.isCurrentUser} key={learner.id} />) : <p className="p-8 text-center font-bold text-text-secondary">No learner results yet.</p>}</div></section>
  </main></StudentShell>;
}

function PodiumLearner({ learner, visualIndex }: { learner: LeaderboardEntry; visualIndex: number }) {
  const first = learner.rank === 1; const heights = ["h-24", "h-36", "h-48", "h-30", "h-20"];
  return <article className="text-center"><div className={`relative mx-auto grid place-items-center overflow-visible rounded-full bg-gradient-to-br from-blue-600 to-violet-600 font-black text-white shadow-xl ring-4 ring-white ${first ? "size-24 text-2xl" : "size-18 text-lg"}`}>{learner.avatar ? <CharacterAvatar avatar={learner.avatar} celebrationSignal={learner.isCurrentUser ? `you-${learner.id}` : first ? `top-${learner.id}` : undefined} className="size-full" label={`${learner.name}'s avatar`} motion={learner.isCurrentUser || first ? "expressive" : "idle"} /> : first ? <Crown className="size-10 text-amber-200" /> : initials(learner.name)}<span className={`absolute -bottom-2 rounded-full px-2 py-0.5 text-xs font-black text-white ${first ? "bg-amber-500" : "bg-violet-700"}`}>#{learner.rank}</span></div><p className="mt-4 truncate text-sm font-black">{learner.name}{learner.isCurrentUser ? " (you)" : ""}</p><p className="text-xs font-bold text-violet-700">{learner.xp} XP</p><div className={`mt-3 flex ${heights[visualIndex]} flex-col items-center justify-start rounded-t-2xl bg-gradient-to-b ${first ? "from-amber-300 to-amber-500" : "from-violet-300 to-violet-500"} pt-3 text-white shadow-inner`}><span className="text-2xl font-black">{learner.rank}</span><span className="mt-1 flex items-center gap-1 text-xs font-bold"><Star className="size-3 fill-current" />{learner.stars}</span></div></article>;
}

function RankingRow({ learner, current }: { learner: LeaderboardEntry; current: boolean }) {
