...
function PodiumLearner({ learner, visualIndex }: { learner: LeaderboardEntry; visualIndex: number }) {
  ...
  return <article className="text-center"><div className={`relative mx-auto grid place-items-center overflow-visible rounded-full bg-gradient-to-br from-blue-600 to-violet-600 font-black text-white shadow-xl ring-4 ring-white ${first ? "size-24 text-2xl" : "size-18 text-lg"}`}><CharacterAvatar avatar={learner.avatar ?? defaultAvatar} celebrationSignal={learner.isCurrentUser ? `you-${learner.id}` : first ? `top-${learner.id}` : undefined} className="size-full" label={`${learner.name}'s avatar`} motion={learner.isCurrentUser || first ? "expressive" : "idle"} /><span className={`absolute -bottom-2 rounded-full px-2 py-0.5 text-xs font-black text-white ${first ? "bg-amber-500" : "bg-violet-700"}`}>#{learner.rank}</span></div><p className="mt-4 truncate text-sm font-black">{learner.name}{learner.isCurrentUser ? " (you)" : ""}</p><p className="text-xs font-bold text-violet-700">{learner.xp} XP</p><div className={`mt-3 flex ${heights[visualIndex]} flex-col items-center justify-start rounded-t-2xl bg-gradient-to-b ${first ? "from-amber-300 to-amber-500" : "from-violet-300 to-violet-500"} pt-3 text-white shadow-inner`}><span className="text-2xl font-black">{learner.rank}</span><span className="mt-1 flex items-center gap-1 text-xs font-bold"><Star className="size-3 fill-current" />{learner.stars}</span></div></article>;
}
