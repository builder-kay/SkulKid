    ...
    <section className="overflow-hidden rounded-[2rem] border border-violet-200 bg-white shadow-[var(--shadow-card)]">
      ...
      <div className="grid gap-5 p-5">
        <section id="leaderboard"><div className="flex items-center justify-between"><div><p className="font-black">Live leaderboard</p><p className="text-xs text-text-secondary">Top learners by total XP</p></div><Crown className="size-5 text-amber-500" /></div>
          {leaderboardLoading ? <p className="mt-4 rounded-xl bg-slate-50 p-4 text-center text-xs font-bold text-text-secondary">Loading standings…</p> : board.length ? <><div className="mt-4 grid grid-cols-3 items-end gap-2">{[topThree[1], topThree[0], topThree[2]].map((player, index) => player ? <div className="text-center" key={player.id}><div className={`mx-auto overflow-hidden rounded-full bg-gradient-to-br from-blue-600 to-violet-600 shadow-sm ring-2 ${index === 1 ? "size-14 ring-amber-400" : "size-11 ring-white"}`}><CharacterAvatar avatar={player.avatar ?? defaultAvatar} className="size-full" label={`${player.name}'s avatar`} motion={index === 1 || player.isCurrentUser ? "expressive" : "idle"} /></div><p className="mt-2 truncate text-xs font-black">{player.name}</p><p className="text-[11px] text-muted">{player.xp} XP</p></div> : null)}</div>
          <ol className="mt-4 grid gap-1.5">{board.slice(0, 5).map((player) => <li className={`flex min-h-9 items-center gap-2 rounded-lg px-2 text-xs ${player.isCurrentUser ? "bg-blue-50 font-black text-blue-900" : "bg-slate-50"}`} key={player.id}><span className="w-7 font-black">#{player.rank}</span><span className="grid size-7 shrink-0 place-items-center overflow-hidden rounded-full bg-gradient-to-br from-blue-600 to-violet-600"><CharacterAvatar avatar={player.avatar ?? defaultAvatar} className="size-full" label={`${player.name}'s avatar`} /></span><span className="flex-1 truncate">{player.name}{player.isCurrentUser ? " (you)" : ""}</span><span className="font-black">{player.xp}</span></li>)}</ol></> : <p className="mt-4 rounded-xl bg-slate-50 p-4 text-center text-xs font-bold text-text-secondary">No learner results yet.</p>}
        </section>
