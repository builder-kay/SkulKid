        ...
        <div className="flex min-h-12 items-center justify-between gap-3">
          <Link className="flex min-w-0 items-center gap-3 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white" href="/admin">
            <span className="min-w-0">
              ...
              <span className="mt-1 block text-xs font-bold text-slate-300">Platform Admin</span>
            </span>
          </Link>
          <button
            aria-label="Sign out"
            className="grid size-11 place-items-center rounded-xl border border-red-900/50 bg-red-950/40 text-red-200"
      ...
      <aside className="sticky top-0 hidden h-screen border-r border-slate-800 bg-slate-950 text-white lg:block">
        <div className="flex h-full flex-col p-5">
          ...
          <nav aria-label="Platform admin navigation" className="mt-6 grid gap-1.5">
            <p className="px-3 pb-1 text-xs font-black uppercase tracking-wider text-slate-500">Control</p>
            {navItems.map((item) => {
