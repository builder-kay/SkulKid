...
export function CurriculumStudio() {
  ...
  useEffect(() => {
    fetch("/api/teacher/curriculum-studio/generate")
  ...
  async function generate(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/teacher/curriculum-studio/generate", { method: "POST", body: form });
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[24rem_1fr]">
        <SkulKidCard className="p-5 sm:p-6">
          <form onSubmit={generate} className="space-y-6">
            ...
            <SkulKidButton type="submit" size="lg" className="w-full" disabled={!file || grades.length === 0 || loading || configured === false}>{loading ? <><LoaderCircle className="size-5 animate-spin" aria-hidden="true" /> Creating subject…</> : <><Sparkles className="size-5" aria-hidden="true" /> Generate draft</>}</SkulKidButton>
