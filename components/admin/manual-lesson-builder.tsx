...
import { saveTopic, saveUnit, useCourses, writeModuleLessonOrder } from "@/lib/courses/course-library";
...
type TeachingBlockPurpose = "hook" | "explain" | "vocabulary" | "example" | "misconception" | "check" | "recap";
type TextLessonSection = { id: string; purpose?: TeachingBlockPurpose; heading: string; body: string; imageUrl: string; imageAlt: string; imageCaption: string; videoUrl: string; videoTitle: string; videoCaption: string };
type CoachSuggestion = { sectionId: string; heading: string; body: string; feedback: string[] };
...
const teachingPurposes: Record<TeachingBlockPurpose, { label: string; prompt: string; heading: string; starter: string }> = {
  hook: { label: "Starter / hook", prompt: "Connect to prior knowledge, curiosity or a familiar Ghanaian context.", heading: "Get ready to learn", starter: "[tip]Think about what you already know about this topic.[/tip]\n\n" },
  explain: { label: "Teach a key idea", prompt: "Explain one important idea using short steps and concrete language.", heading: "Learn the key idea", starter: "" },
  vocabulary: { label: "Key vocabulary", prompt: "Introduce only the words and symbols pupils need, with plain definitions.", heading: "Words to know", starter: "- **Key word:** simple meaning\n" },
  example: { label: "Concrete example", prompt: "Connect the idea to a familiar situation before moving to the abstract.", heading: "See the idea", starter: "For example, \n" },
  misconception: { label: "Common mistake", prompt: "Name a likely misconception and clearly contrast it with the correct idea.", heading: "Watch out for this", starter: "[tip]A common mistake is…[/tip]\n\nThe correct idea is…\n" },
  check: { label: "Check understanding", prompt: "Ask pupils to retrieve, explain or apply the idea before moving on.", heading: "Pause and think", starter: "Try this:\n\n" },
  recap: { label: "Recap / reflection", prompt: "Restate the essential learning and invite pupils to reflect.", heading: "What to remember", starter: "- \n" }
};
...
type FormState = {
  ...
  courseId: string; unitId: string; topicId: string;
  title: string; description: string; curriculumReference: string; contentStandard: string; indicator: string; lessonNumber: number; objectives: string[];
...
const initial: FormState = {
  lessonFormat: "text", classId: "", subject: "mathematics", grade: 4, unit: "", chapter: "", topic: "", prerequisiteLessonId: "", courseId: "", unitId: "", topicId: "", title: "", description: "", curriculumReference: "", contentStandard: "", indicator: "", lessonNumber: 1,
  ...
  exampleProblem: "", exampleSteps: "", exampleAnswer: "", videos: [{ id: "video-1", url: "", title: "Watch and learn", caption: "", participationPrompt: "", participationOptionA: "", participationOptionB: "", participationOptionC: "", participationCorrectOption: 0, participationExplanation: "", participationXp: 5 }], textSections: [{ id: "text-section-1", purpose: "hook", heading: "Get ready to learn", body: "", imageUrl: "", imageAlt: "", imageCaption: "", videoUrl: "", videoTitle: "Watch and learn", videoCaption: "" }], questions: [{ id: "question-1", prompt: "", optionA: "", optionB: "", optionC: "", correctOption: 0, hint: "", explanation: "" }],
...
export function ManualLessonBuilder({ initialAiConfigured = false, initialAiModel = "Not configured", editLessonId, initialCourseId, initialClassId }: { initialAiConfigured?: boolean; initialAiModel?: string; editLessonId?: string; initialCourseId?: string; initialClassId?: string }) {
  ...
  const studioChecks = [
    Boolean(form.courseId && form.unit.trim() && form.chapter.trim() && form.topic.trim()),
    Boolean(form.title.trim().length >= 3 && form.description.trim()),
    form.objectives.some((objective) => objective.trim().length >= 3),
    Boolean(hasText || hasVideo),
    completeQuestions.length > 0
  ];
  const studioProgress = Math.round((studioChecks.filter(Boolean).length / studioChecks.length) * 100);
  ...
  async function createPlacementSubject() {
    ...
    try {
      ...
      setForm((current) => ({ ...current, courseId: payload.id!, unitId: "__new__", topicId: "", unit: "", chapter: "" }));
      setNewSubjectName("");
  ...
  useEffect(() => {
    ...
    setForm((current) => {
      if (current.courseId === subjectCourse.id) {
        if (!subjectCourse.units.length && !current.unitId) return { ...current, unitId: "__new__", topicId: "" };
        return current;
      }
  ...
  function addTextSection(purpose: TeachingBlockPurpose = "explain") { const preset = teachingPurposes[purpose]; setForm((current) => ({ ...current, textSections: [...current.textSections, { id: crypto.randomUUID(), purpose, heading: preset.heading, body: preset.starter, imageUrl: "", imageAlt: "", imageCaption: "", videoUrl: "", videoTitle: "Watch and learn", videoCaption: "" }] })); setResult(null); }
  ...
  async function coachTextSection(section: TextLessonSection, action: "draft" | "improve" | "simplify" | "review") {
    setCoaching(`${section.id}:${action}`); setCoachError(""); setCoachSuggestion(null);
    try {
      const response = await fetch("/api/teacher/lessons/coach", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action, purpose: section.purpose ?? "explain", subject: form.subject, grade: form.grade, lessonTitle: form.title, objectives: form.objectives, heading: section.heading, body: section.body })
      });
      const payload = await response.json() as { suggestion?: Omit<CoachSuggestion, "sectionId">; error?: string };
      if (!response.ok || !payload.suggestion) throw new Error(payload.error || "The lesson coach could not respond.");
      setCoachSuggestion({ sectionId: section.id, ...payload.suggestion });
    } catch (cause) { setCoachError(cause instanceof Error ? cause.message : "The lesson coach could not respond."); }
    finally { setCoaching(null); }
  }
  ...
  function applyImportedLesson(data: ImportedLesson) {
    setForm((current) => ({ ...current, unit: data.unit, chapter: data.chapter || data.unit, topic: data.topic, title: data.title,
      ...
      heading: data.teachingHeading, body: data.teachingText, textSections: [{ ...initial.textSections[0], id: crypto.randomUUID(), purpose: "explain", heading: data.teachingHeading, body: data.teachingText }], exampleTitle: data.exampleTitle, exampleProblem: data.exampleProblem,
  ...
  function build() {
    ...
    if (!form.chapter.trim()) { setMessage("Enter the curriculum sub-strand."); return null; }
    if (!form.topic.trim()) { setMessage("Enter the lesson topic."); return null; }
    if (!Number.isInteger(form.lessonNumber) || form.lessonNumber < 1) { setMessage("Lesson number must be 1 or greater."); return null; }
    ...
    const course = generatedCourseSchema.safeParse({
      ...
      sourceSummary: `Teacher lesson for Basic ${form.grade}, strand ${unit}, sub-strand ${chapter}, topic ${topic}.`,
      designRationale: "Administrator-authored short mission with instruction, worked practice, assessment and retrieval summary.",
      units: [{ title: unit, description: `Basic ${form.grade} curriculum strand: ${unit}.`, grade: form.grade, topics: [{ title: topic, description: `Learn and practise ${topic} in this lesson.`, lessons: [{
  ...
  async function saveLesson(status: AdminLessonStatus) {
    ...
    if (!topicId || topicId === "__new__") {
      if (!form.chapter.trim()) { setMessage("Enter a sub-strand name before saving."); setSavingStatus(null); return; }
      topicId = await saveTopic(unitId, {
        title: form.chapter.trim(),
        description: `Sub-strand under ${unitTitle}.`
      });
    }
    ...
    const saveResult = await writeAdminLesson({
      ...
      topic: form.topic,
      contentStandard: form.contentStandard.trim() || undefined,
      ...
      updatedAt: now,
      fixture,
      builderState: structuredClone({ ...form, courseId, unitId, topicId, unit: unitTitle })
    });
  ...
  return <LessonFormatContext.Provider value={form.lessonFormat}><main data-lesson-format={form.lessonFormat} className="mx-auto w-full max-w-[100rem] [&[data-lesson-format=text]_.video-only-fields]:hidden [&[data-lesson-format=video]_.text-lesson-fields]:hidden [&[data-lesson-format=video]_.ai-text-only]:hidden">
    <header className="relative overflow-hidden rounded-[1.75rem] border border-slate-800 bg-slate-950 px-5 py-6 text-white shadow-[0_22px_60px_rgba(15,23,42,.18)] sm:px-7 lg:px-8"><div className="pointer-events-none absolute -right-16 -top-32 size-80 rounded-full bg-violet-500/25 blur-3xl" /><div className="relative flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between"><div className="flex items-start gap-4"><span className="hidden size-12 shrink-0 place-items-center rounded-2xl bg-violet-500/20 text-violet-200 ring-1 ring-violet-400/30 sm:grid"><FileText className="size-6" /></span><div><div className="flex flex-wrap items-center gap-2"><p className="text-[11px] font-black uppercase tracking-[0.2em] text-violet-300">Lesson authoring studio</p><span className="rounded-full bg-white/10 px-2 py-1 text-[10px] font-black uppercase text-slate-300">{editLessonId ? "Editing lesson" : "New lesson"}</span></div><h1 className="mt-2 text-2xl font-black sm:text-3xl">{form.title.trim() || "Create a pupil-ready lesson"}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Plan the curriculum path, build focused learning blocks, check understanding and publish when ready.</p></div></div><div className="flex flex-wrap items-center gap-2"><SkulKidButton type="button" variant="ghost" className="text-slate-200 hover:bg-white/10 hover:text-white" onClick={restore}>Restore saved draft</SkulKidButton><SkulKidButton type="button" variant="outline" className="border-white/25 bg-white/10 text-white hover:bg-white/20" onClick={saveLocally}><Save className="size-4" />Save draft</SkulKidButton></div></div></header>
      ...
      <div className="min-w-0 space-y-6">
        <Section number="1" title="Curriculum placement" description="Place the lesson under its Ghana curriculum strand, sub-strand and topic.">
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Subject">
              <Select value={form.courseId || subjectCourse?.id || ""} onChange={(e) => {
                ...
                const supported = course?.slug === "mathematics" || course?.slug === "science" || course?.slug === "english-language"
                  ...
                  : form.subject;
                setForm((current) => ({ ...current, subject: supported, courseId: e.target.value, unitId: course?.units.length ? "" : "__new__", topicId: "", unit: "", chapter: "" }));
                setResult(null);
              }}>
            ...
            <Field label="Strand">
              ...
              >
                ...
                {moduleOptions.map((unit, index) => (
                  <option key={unit.id} value={unit.id}>Strand {index + 1}: {unit.title}</option>
                ))}
                <option value="__new__">Create a new strand…</option>
              </Select>
            </Field>
            <Field label="Class assignment (optional)">
              <Select value={form.classId} onChange={(event) => {
                ...
                setForm((current) => {
                  ...
                  return { ...current, classId: nextClassId, ...(courseAllowed ? {} : { courseId: "", unitId: "", topicId: "", unit: "", chapter: "" }) };
                });
                setResult(null);
              }}>
            ...
            {form.unitId === "__new__" || !moduleOptions.length ? (
              <Field label={form.unitId === "__new__" ? "New strand name" : "Strand name"}>
                <Input
                  value={form.unit}
                  onChange={(e) => {
            ...
            {form.unitId && form.unitId !== "__new__" ? <Field label="Sub-strand">
              <Select value={form.topicId} onChange={(event) => {
                const nextId = event.target.value;
                const selected = subStrandOptions.find((item) => item.id === nextId);
                setForm((current) => ({ ...current, topicId: nextId, chapter: nextId === "__new__" ? "" : selected?.title ?? "" }));
                setResult(null);
              }} required>
                <option value="">Choose a sub-strand</option>
                {subStrandOptions.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}
                <option value="__new__">Create a new sub-strand…</option>
              </Select>
            </Field> : null}
            {form.unitId === "__new__" || !form.unitId || form.topicId === "__new__" || (!subStrandOptions.length && form.unitId !== "__new__") ? <Field label="New sub-strand name">
              <Input value={form.chapter} onChange={(e) => update("chapter", e.target.value)} placeholder="e.g. Living and Non-Living Things" required />
            </Field> : null}
            <Field label="Topic">
              <Input value={form.topic} onChange={(e) => update("topic", e.target.value)} placeholder="e.g. The Root Systems of Plants" required />
            </Field>
          </div>
          {subjectCourse ? (
            <p className="mt-3 text-xs font-bold text-muted">
              Placement: {teacherClasses.find((item) => item.id === form.classId)?.name ?? "Public Learning course"} → {subjectCourse.name} → {moduleOptions.find((item) => item.id === form.unitId)?.title ?? (form.unit || "No strand selected")} → {form.chapter || "No sub-strand"} → {form.topic || "No topic"}.
            </p>
          ) : (
            <p className="mt-3 text-xs font-bold text-amber-800">
              No catalogue subject found for this selection yet. Create strands under Curriculum, or enter a new strand name here.
            </p>
          )}
        </Section>
        ...
        <Section number="2" title="Lesson foundations" description="State what pupils will learn and connect it to the Ghana curriculum."><div className="grid gap-5"><div className="grid gap-4 sm:grid-cols-[1fr_9rem]"><Field label="Lesson title"><Input value={form.title} onChange={(e) => update("title", e.target.value)} placeholder="e.g. Classification of Plants Based on Their Root Systems" required /></Field><Field label="Lesson number"><Input type="number" min={1} value={form.lessonNumber} onChange={(e) => update("lessonNumber", Number(e.target.value))} required /></Field></div><Field label="Short description"><Textarea value={form.description} onChange={(e) => update("description", e.target.value)} placeholder="What will happen in this lesson?" required /></Field>
          ...
          <div className="grid gap-4 rounded-2xl border border-emerald-200 bg-emerald-50/60 p-4 sm:p-5">
            ...
            <Field label="Indicator (optional)"><Textarea value={form.indicator} onChange={(e) => update("indicator", e.target.value)} placeholder="B6.1.1.1.1: Classify plants based on their root systems." /></Field>
          </div>
        ...
        <Section number="3" title="Teaching material" description="Guide pupils from curiosity to understanding with focused learning blocks.">
          <TeachingMaterialStudio
            aiConfigured={aiConfigured}
            coachError={coachError}
            coaching={coaching}
            coachSuggestion={coachSuggestion}
            onAcceptSuggestion={(suggestion) => { updateTextSection(suggestion.sectionId, { heading: suggestion.heading, body: suggestion.body }); setCoachSuggestion(null); }}
            onAdd={addTextSection}
            onCoach={coachTextSection}
            onDismissSuggestion={() => setCoachSuggestion(null)}
            onFormat={format}
            onMove={moveTextSection}
            onPreviewChange={setMaterialPreview}
            onRemove={removeTextSection}
            onUpdate={updateTextSection}
            onEditorShortcut={handleEditorShortcut}
            preview={materialPreview}
            sections={form.textSections}
            wordCount={wordCount}
          />
        </Section>
...
function TeachingMaterialStudio({
  sections, preview, wordCount, aiConfigured, coaching, coachSuggestion, coachError,
  onPreviewChange, onUpdate, onAdd, onRemove, onMove, onFormat, onEditorShortcut, onCoach, onAcceptSuggestion, onDismissSuggestion
}: {
  sections: TextLessonSection[]; preview: boolean; wordCount: number; aiConfigured: boolean; coaching: string | null; coachSuggestion: CoachSuggestion | null; coachError: string;
  onPreviewChange: (value: boolean) => void; onUpdate: (id: string, changes: Partial<TextLessonSection>) => void; onAdd: (purpose: TeachingBlockPurpose) => void;
  onRemove: (id: string) => void; onMove: (index: number, direction: -1 | 1) => void; onFormat: (action: FormatAction, id: string) => void;
  onEditorShortcut: (event: React.KeyboardEvent<HTMLTextAreaElement>, id: string) => void;
  onCoach: (section: TextLessonSection, action: "draft" | "improve" | "simplify" | "review") => void;
  onAcceptSuggestion: (suggestion: CoachSuggestion) => void; onDismissSuggestion: () => void;
}) {
  return <div className="grid gap-5">
    <div className="rounded-2xl border border-violet-200 bg-gradient-to-r from-violet-50 to-blue-50 p-4 sm:p-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div><h3 className="font-black text-slate-950">Build the learning journey</h3><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-600">Use short, purposeful blocks. A strong lesson usually starts with prior knowledge, teaches one idea at a time, models it, checks understanding and ends with a recap.</p></div>
        <button aria-pressed={preview} className={`inline-flex min-h-11 items-center gap-2 rounded-xl px-4 text-sm font-black transition ${preview ? "bg-violet-700 text-white" : "border border-violet-300 bg-white text-violet-800"}`} onClick={() => onPreviewChange(!preview)} type="button"><Eye className="size-4" />{preview ? "Return to editor" : "Preview as learner"}</button>
      </div>
      <div className="mt-4 flex flex-wrap gap-2">{(Object.entries(teachingPurposes) as Array<[TeachingBlockPurpose, typeof teachingPurposes[TeachingBlockPurpose]]>).map(([purpose, item]) => <button className="min-h-10 rounded-xl border border-white bg-white px-3 text-xs font-black text-slate-700 shadow-sm hover:border-violet-300 hover:text-violet-800" key={purpose} onClick={() => onAdd(purpose)} type="button"><Plus className="mr-1 inline size-3.5" />{item.label}</button>)}</div>
    </div>

    {preview ? <div className="mx-auto w-full max-w-3xl rounded-[2rem] border-8 border-slate-900 bg-slate-100 p-3 shadow-2xl sm:p-5"><div className="mb-4 flex items-center justify-between rounded-xl bg-slate-900 px-4 py-2 text-xs font-black text-white"><span>Learner preview</span><span>Mobile lesson view</span></div><div className="grid gap-4">{sections.filter((section) => section.body.trim()).map((section, index) => <article className="overflow-hidden rounded-[1.5rem] border border-violet-100 bg-white p-5 shadow-sm sm:p-6" key={section.id}><div className="mb-4 flex items-center justify-between gap-3"><span className="rounded-full bg-violet-100 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-violet-800">{teachingPurposes[section.purpose ?? "explain"].label}</span><span className="text-xs font-bold text-slate-400">{index + 1} of {sections.filter((item) => item.body.trim()).length}</span></div><h3 className="text-2xl font-black text-slate-950">{section.heading}</h3><FormattedText className="mt-4" size="lesson" text={section.body} />{section.imageUrl ? <figure className="mt-5"><img alt={section.imageAlt || "Lesson illustration"} className="max-h-80 w-full rounded-2xl object-contain" src={section.imageUrl} />{section.imageCaption ? <figcaption className="mt-2 text-center text-xs text-slate-500">{section.imageCaption}</figcaption> : null}</figure> : null}{resolveVideoEmbed(section.videoUrl) ? <div className="mt-5"><EmbeddedVideo url={section.videoUrl} title={section.videoTitle || section.heading} caption={section.videoCaption} /></div> : null}</article>)}</div>{!sections.some((section) => section.body.trim()) ? <p className="p-8 text-center text-sm font-bold text-slate-500">Start writing a block to see the learner preview.</p> : null}</div> :
    <div className="grid gap-5">{sections.map((section, index) => {
      const purpose = section.purpose ?? "explain"; const guide = teachingPurposes[purpose]; const sectionWords = section.body.trim() ? section.body.trim().split(/\s+/).length : 0; const sectionVideo = section.videoUrl.trim() ? resolveVideoEmbed(section.videoUrl) : null; const suggestion = coachSuggestion?.sectionId === section.id ? coachSuggestion : null;
      return <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" key={section.id}>
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3"><div className="flex min-w-0 items-center gap-3"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-violet-700 text-sm font-black text-white">{index + 1}</span><div><select aria-label={`Purpose of block ${index + 1}`} className="rounded-lg border-0 bg-transparent p-0 text-sm font-black text-slate-950 focus:ring-2 focus:ring-violet-500" onChange={(event) => onUpdate(section.id, { purpose: event.target.value as TeachingBlockPurpose })} value={purpose}>{(Object.entries(teachingPurposes) as Array<[TeachingBlockPurpose, typeof guide]>).map(([value, item]) => <option key={value} value={value}>{item.label}</option>)}</select><p className="mt-0.5 text-xs text-slate-500">{guide.prompt}</p></div></div><div className="flex gap-1"><SkulKidButton type="button" size="icon" variant="ghost" disabled={index === 0} onClick={() => onMove(index, -1)} aria-label={`Move block ${index + 1} up`}><ArrowUp className="size-4" /></SkulKidButton><SkulKidButton type="button" size="icon" variant="ghost" disabled={index === sections.length - 1} onClick={() => onMove(index, 1)} aria-label={`Move block ${index + 1} down`}><ArrowDown className="size-4" /></SkulKidButton><SkulKidButton type="button" size="icon" variant="ghost" disabled={sections.length === 1} onClick={() => onRemove(section.id)} aria-label={`Remove block ${index + 1}`}><Trash2 className="size-4" /></SkulKidButton></div></header>
        <div className="grid gap-4 p-4 sm:p-5">
          <div className="flex flex-wrap items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 p-2" role="toolbar" aria-label={`Formatting tools for block ${index + 1}`}><Tool label="Heading" icon={Heading2} onClick={() => onFormat("heading", section.id)} /><Tool label="Bold" shortcut="Ctrl+B" icon={Bold} onClick={() => onFormat("bold", section.id)} /><Tool label="Italic" shortcut="Ctrl+I" icon={Italic} onClick={() => onFormat("italic", section.id)} /><Tool label="Bullet list" icon={List} onClick={() => onFormat("bullet", section.id)} /><Tool label="Numbered list" icon={ListOrdered} onClick={() => onFormat("numbered", section.id)} /><Tool label="Teaching tip" icon={Lightbulb} onClick={() => onFormat("tip", section.id)} /><Tool label="Link" icon={Link2} onClick={() => onFormat("link", section.id)} /><Tool label="Clear formatting" icon={RemoveFormatting} onClick={() => onFormat("clear", section.id)} /></div>
          <Field label="Block heading"><Input value={section.heading} onChange={(event) => onUpdate(section.id, { heading: event.target.value })} placeholder={guide.heading} /></Field>
          <Field label="What pupils will read"><Textarea id={`lesson-body-${section.id}`} className="min-h-52 bg-white text-[16px] leading-7" value={section.body} onChange={(event) => onUpdate(section.id, { body: event.target.value })} onKeyDown={(event) => onEditorShortcut(event, section.id)} placeholder={guide.prompt} /></Field>
          <div className="flex flex-wrap items-center justify-between gap-2 text-xs font-bold text-slate-500"><span>{sectionWords} words · about {Math.max(1, Math.ceil(sectionWords / 130))} min read</span><span className={sectionWords > 250 ? "text-amber-700" : "text-emerald-700"}>{sectionWords > 250 ? "Consider splitting this block" : "Focused block length"}</span></div>
          <div className="rounded-xl border border-violet-200 bg-violet-50 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><WandSparkles className="size-4 text-violet-700" /><span className="text-sm font-black text-violet-950">AI teaching coach</span></div><div className="flex flex-wrap gap-1">{(["draft", "improve", "simplify", "review"] as const).map((action) => <button className="min-h-9 rounded-lg bg-white px-3 text-xs font-black capitalize text-violet-800 shadow-sm disabled:opacity-50" disabled={!aiConfigured || Boolean(coaching) || (action !== "draft" && !section.body.trim())} key={action} onClick={() => void onCoach(section, action)} type="button">{coaching === `${section.id}:${action}` ? <LoaderCircle className="inline size-3.5 animate-spin" /> : action}</button>)}</div></div>{!aiConfigured ? <p className="mt-2 text-xs text-slate-600">AI coaching is unavailable until Gemini is configured. All guided tools still work without it.</p> : null}</div>
          {suggestion ? <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p className="font-black text-emerald-950">Coach suggestion — review before applying</p><h4 className="mt-3 font-black">{suggestion.heading}</h4><FormattedText className="mt-2 text-sm" text={suggestion.body} />{suggestion.feedback.length ? <ul className="mt-3 list-disc space-y-1 pl-5 text-xs text-emerald-900">{suggestion.feedback.map((item) => <li key={item}>{item}</li>)}</ul> : null}<div className="mt-3 flex gap-2"><button className="min-h-10 rounded-lg bg-emerald-700 px-4 text-xs font-black text-white" onClick={() => onAcceptSuggestion(suggestion)} type="button">Apply suggestion</button><button className="min-h-10 rounded-lg border border-emerald-300 bg-white px-4 text-xs font-black text-emerald-900" onClick={onDismissSuggestion} type="button">Keep my version</button></div></div> : null}
          <div className="grid gap-3 sm:grid-cols-2"><details className="rounded-xl border border-blue-200 bg-blue-50 p-4"><summary className="cursor-pointer font-black text-blue-950"><FileImage className="mr-2 inline size-4" />Add image</summary><div className="mt-4 grid gap-3"><Field label="Image URL"><Input type="url" value={section.imageUrl} onChange={(event) => onUpdate(section.id, { imageUrl: event.target.value })} /></Field><Field label="Alternative text"><Input value={section.imageAlt} onChange={(event) => onUpdate(section.id, { imageAlt: event.target.value })} placeholder="Describe what the image teaches" /></Field><Field label="Caption (optional)"><Input value={section.imageCaption} onChange={(event) => onUpdate(section.id, { imageCaption: event.target.value })} /></Field></div></details><details className="rounded-xl border border-sky-200 bg-sky-50 p-4"><summary className="cursor-pointer font-black text-sky-950"><Video className="mr-2 inline size-4" />Add supporting video</summary><div className="mt-4 grid gap-3"><Field label="Public video link"><Input type="url" value={section.videoUrl} onChange={(event) => onUpdate(section.id, { videoUrl: event.target.value })} aria-invalid={Boolean(section.videoUrl && !sectionVideo)} /></Field><Field label="Video title"><Input value={section.videoTitle} onChange={(event) => onUpdate(section.id, { videoTitle: event.target.value })} /></Field><Field label="Viewing instruction"><Input value={section.videoCaption} onChange={(event) => onUpdate(section.id, { videoCaption: event.target.value })} placeholder="Tell pupils what to notice" /></Field>{section.videoUrl && !sectionVideo ? <p className="text-xs font-bold text-red-700">Use a valid public YouTube, Vimeo or TikTok link.</p> : null}</div></details></div>
        </div>
      </article>;
    })}</div>}
    {coachError ? <p role="alert" className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm font-bold text-rose-900">{coachError}</p> : null}
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-50 p-3 text-xs font-bold text-slate-600"><span>{wordCount} words across {sections.length} learning block{sections.length === 1 ? "" : "s"}</span><span>Suggested total: {Math.max(250, sections.length * 100)}–{Math.max(500, sections.length * 220)} words</span></div>
  </div>;
}
...
function LessonSectionNav({ format, progress }: { format: LessonFormat; progress: number }) {
  const items = [
    ...
    { id: "course-placement", label: "Curriculum placement" },
    { id: "learning-objectives", label: "Lesson foundations" },
...
function formatCourseIssue(issue: { path: PropertyKey[]; message: string } | undefined) {
  ...
  const fieldNames: Record<string, string> = {
    ...
    summaryPoints: "Summary points", subject: "Subject", unit: "Strand", chapter: "Sub-strand", topic: "Topic"
  };
