...
export function CourseManagement() {
  ...
  async function changeStatus(course: ManagedCourse) {
    ...
    try {
      ...
      await refresh();
      setMessage(course.status === "published" ? "Subject unpublished." : "Subject is now live for pupils.");
    } catch (cause) {
    ...
    <section className="mt-6 grid gap-5" aria-labelledby="course-management-heading">
      <SkulKidCard className="overflow-hidden">
        <div className="flex flex-col gap-4 bg-gradient-to-r from-slate-950 via-violet-950 to-violet-800 p-6 text-white sm:flex-row sm:items-end sm:justify-between sm:p-8">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-violet-200">Live subject management</p>
            <h2 className="mt-2 text-3xl font-black" id="course-management-heading">Build the student catalogue</h2>
            <p className="mt-2 max-w-2xl text-violet-100">Create subject cards, organise modules and topics, attach lessons and control what pupils can see.</p>
          </div>
          <button className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-white px-5 font-black text-slate-950 shadow-lg hover:bg-violet-50" onClick={() => setForm(emptyForm)} type="button"><Plus className="size-5" />Create subject</button>
        </div>
          ...
          <div className="grid lg:grid-cols-[20rem_1fr]">
            <div className="border-b border-slate-200 bg-slate-50 p-4 lg:border-b-0 lg:border-r">
              <div className="grid gap-2">
                ...
                {!courses.length ? <p className="p-5 text-center text-sm font-bold text-muted">Create the first subject to begin.</p> : null}
              </div>
            </div>
...
function CourseWorkspace({ course, lessons, saving, onEdit, onStatus, onRefresh, setMessage }: { course: ManagedCourse; lessons: AdminLessonRecord[]; saving: boolean; onEdit: () => void; onStatus: () => void; onRefresh: () => Promise<void>; setMessage: (value: string) => void }) {
  ...
  return <div className="grid gap-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      ...
      <div className="flex shrink-0 gap-2"><button className="grid size-11 place-items-center rounded-xl border border-slate-300 hover:bg-slate-50" aria-label="Edit subject" onClick={onEdit} type="button"><Pencil className="size-4" /></button><button className={`inline-flex min-h-11 items-center gap-2 rounded-xl px-4 text-sm font-black ${course.status === "published" ? "bg-amber-100 text-amber-950" : "bg-emerald-600 text-white"}`} disabled={saving} onClick={onStatus} type="button">{course.status === "published" ? <EyeOff className="size-4" /> : <Eye className="size-4" />}{course.status === "published" ? "Unpublish" : "Publish"}</button></div>
    </div>
