...
import { ArrowDown, ArrowUp, BookOpen, CheckCircle2, Clock3, Eye, FilePenLine, Filter, GripVertical, Plus, Search, Trophy } from "lucide-react";
...
export function LessonLibrary({ initialSubject }: { initialSubject: SupportedCurriculumSubject }) {
  ...
  const existingLessons = useMemo<AdminLessonRecord[]>(() => sampleLessons.map((lesson) => ({
    id: lesson.id, subject: subjectSlug(lesson.subjectId), grade: gradeFromUnit(lesson.unitId), unit: unitName(lesson.unitId),
    chapter: "Published curriculum", topic: topicName(lesson.topicId), title: lesson.title, description: lesson.description,
    estimatedMinutes: lesson.estimatedMinutes, xp: lesson.xpReward, questionCount: lesson.blocks.filter((block) => block.type === "multiple_choice" || block.type === "true_false").length,
    format: "text", status: "published", createdAt: lesson.createdAt, updatedAt: lesson.updatedAt, fixture: null
  })), []);
  const allLessons = useMemo(() => [...savedLessons, ...existingLessons.filter((lesson) => !savedLessons.some((saved) => saved.id === lesson.id))], [savedLessons, existingLessons]);
...
function subjectSlug(subjectId: string): SupportedCurriculumSubject { return subjectId.includes("english") ? "english-language" : subjectId.includes("science") ? "science" : "mathematics"; }
function gradeFromUnit(unitId: string) { return Number(unitId.match(/basic-(\d)/)?.[1] ?? 4); }
function unitName(unitId: string) { return unitId.includes("english") ? `Basic ${gradeFromUnit(unitId)}` : "Fractions"; }
function topicName(topicId: string) { return topicId.split("-").slice(-2).map((word) => word[0]?.toUpperCase() + word.slice(1)).join(" "); }
