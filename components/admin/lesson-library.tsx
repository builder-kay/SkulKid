...
export function LessonLibrary({ initialSubject, initialCourseId }: { initialSubject: SupportedCurriculumSubject; initialCourseId?: string }) {
  ...
  async function saveSubjectAccess() {
    ...
    try {
      ...
      await loadAccess();
      setAccessMessage("Subject audience updated. Every strand and lesson follows the same setting.");
    } catch (cause) {
  ...
  const moduleGroups = [
    ...
    }).filter((group) => group.lessons.length > 0),
    {
      ...
      description: "Lessons that have not yet been placed inside a strand.",
  ...
  return <main className="mx-auto w-full max-w-[94rem] space-y-6">
    ...
    <SkulKidCard className="p-4 sm:p-5"><div className="grid gap-3 sm:grid-cols-[1fr_13rem]"><label className="relative"><Search className="pointer-events-none absolute left-4 top-3.5 size-4 text-muted" /><Input className="pl-11" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search lessons, strands or topics" aria-label="Search lessons" /></label><label className="relative"><Filter className="pointer-events-none absolute left-4 top-3.5 size-4 text-muted" /><Select className="pl-11" value={status} onChange={(event) => setStatus(event.target.value as typeof status)} aria-label="Filter by status"><option value="all">All statuses</option><option value="published">Published</option><option value="draft">Drafts</option></Select></label></div></SkulKidCard>
