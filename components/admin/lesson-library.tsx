...
export function LessonLibrary({ initialSubject, initialCourseId }: { initialSubject: SupportedCurriculumSubject; initialCourseId?: string }) {
  ...
  return <main className="mx-auto w-full max-w-[94rem] space-y-6">
    <header className="relative overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-xl sm:p-8 lg:p-10"><div className="pointer-events-none absolute -right-20 -top-24 size-80 rounded-full bg-violet-600 opacity-20 blur-3xl" /><div className="relative flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-slate-300">Lesson library</p><h1 className="mt-3 text-3xl font-black sm:text-5xl">{activeCourse?.name ?? legacySubject?.name ?? "Course"} lessons</h1><p className="mt-4 max-w-2xl text-base leading-7 text-slate-300">Lessons are organised under their assigned modules. Reorder lessons without moving them out of their module.</p></div><div className="flex flex-col gap-3 sm:flex-row"><TeacherGuideLink className="border-white/20 bg-white/10 text-white hover:border-white/40 hover:bg-white/20 hover:text-white" topic="link-lessons" /><Link href={`/teacher/lessons/new?${activeCourse?.ownerClassId ? `classId=${encodeURIComponent(activeCourse.ownerClassId)}&` : ""}courseId=${encodeURIComponent(courseId)}`} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-white px-5 font-black text-slate-950 shadow-lg hover:bg-slate-100"><Plus className="size-5" />Create another lesson</Link></div></div></header>
