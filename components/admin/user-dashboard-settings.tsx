...
const storageKey = "skulkid-user-dashboard-settings";
...
export function UserDashboardSettings() {
  ...
  useEffect(() => {
    ...
    if (stored) setSettings({ ...defaults, ...(JSON.parse(stored) as Partial<DashboardSettings>) });
  }, []);
  ...
  function save() { window.localStorage.setItem(storageKey, JSON.stringify(settings)); setSaved(true); }
  function reset() { setSettings(defaults); window.localStorage.removeItem(storageKey); setSaved(false); }
