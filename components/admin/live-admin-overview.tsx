...
import { ArrowRight, BookOpen, BookOpenCheck, Calculator, Clock3, FileCheck2, FlaskConical, Layers3, Pencil, PlayCircle, Plus, Sparkles } from "lucide-react";
...
export function LiveCurriculumCoverage() {
  const lessons = useLiveLessons();
  return <SkulKidCard className="overflow-hidden"><div className="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"><div><p className="text-xs font-black uppercase tracking-wider text-violet-700">Curriculum coverage</p><h2 className="mt-1 text-2xl font-black">Subjects ready for pupils</h2></div><Link href="/teacher/curriculum" className="inline-flex min-h-11 items-center gap-2 text-sm font-bold text-violet-700 hover:text-violet-900">Manage curriculum<ArrowRight className="size-4" /></Link></div><div className="grid gap-4 p-5 sm:p-6 md:grid-cols-3">{subjectDetails.map((subject) => {
    ...
    return <article className="rounded-2xl border border-slate-200 bg-slate-50 p-4" key={subject.id}><div className="flex items-center justify-between gap-3"><span className="grid size-10 place-items-center rounded-xl bg-white text-lg font-black shadow-sm" style={{ color: subject.colour }}>{subject.name.charAt(0)}</span><span className={`rounded-full px-2.5 py-1 text-xs font-black ${published ? "bg-green-100 text-green-900" : drafts ? "bg-blue-100 text-blue-900" : "bg-amber-100 text-amber-950"}`}>{published ? "Live" : drafts ? "Draft only" : "Not started"}</span></div><h3 className="mt-4 text-lg font-black">{subject.name}</h3><dl className="mt-4 grid grid-cols-2 gap-2 text-center"><Count label="Published" value={published} /><Count label="Drafts" value={drafts} /></dl><Link href={records.length ? "/teacher/curriculum" : "/teacher/lessons/new"} className="mt-4 inline-flex min-h-10 items-center gap-2 text-sm font-bold text-violet-700 hover:text-violet-900">{records.length ? "Open subject" : "Create first lesson"}<ArrowRight className="size-4" /></Link></article>;
  })}</div></SkulKidCard>;
}
...
export function LiveAdminCurriculumSubjects() {
  ...
  return <section aria-label="Subjects" className="mt-6 grid gap-5 lg:grid-cols-3">{display.map((subject) => { const Icon = subject.icon; const records = lessons.filter((lesson) => lesson.subject === subject.id); const published = records.filter((lesson) => lesson.status === "published"); const units = new Set(published.map((lesson) => lesson.unit.trim()).filter(Boolean)).size; const topics = new Set(published.map((lesson) => lesson.topic.trim()).filter(Boolean)).size; return <SkulKidCard key={subject.id} className="flex flex-col p-6"><div className="flex items-start justify-between gap-3"><span className={`grid size-12 place-items-center rounded-2xl ${subject.soft}`}><Icon className="size-6" /></span><span className={`rounded-full px-3 py-1 text-xs font-black ${published.length ? "bg-green-100 text-green-900" : "bg-amber-100 text-amber-900"}`}>{published.length ? "Available" : "Needs content"}</span></div><h2 className="mt-5 text-2xl font-black">{subject.name}</h2><p className="mt-2 min-h-14 leading-7 text-text-secondary">{subject.description}</p><dl className="mt-5 grid grid-cols-3 gap-2 text-center"><Count label="Strands" value={units} /><Count label="Topics" value={topics} /><Count label="Published" value={published.length} /></dl>{records.length > published.length ? <p className="mt-3 text-center text-xs font-bold text-amber-800">{records.length - published.length} draft{records.length - published.length === 1 ? "" : "s"} not visible to pupils</p> : null}<div className="mt-6 grid gap-2"><Link href={`/courses/${subject.id}`} className="inline-flex min-h-11 items-center justify-between rounded-xl border border-slate-200 px-4 font-bold hover:bg-slate-50"><span className="flex items-center gap-2"><PlayCircle className="size-4" />Student subject</span><ArrowRight className="size-4" /></Link><Link href={`/teacher/lessons/new`} className="inline-flex min-h-11 items-center justify-between rounded-xl bg-violet-50 px-4 font-bold text-violet-800 hover:bg-violet-100"><span className="flex items-center gap-2"><Sparkles className="size-4" />Create lesson</span><ArrowRight className="size-4" /></Link></div></SkulKidCard>; })}</section>;
}
