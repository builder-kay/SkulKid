  ...
  return <section className="grid gap-5">
    ...
    <form className="rounded-[1.5rem] border border-blue-200 bg-white p-5 shadow-sm sm:p-6" onSubmit={invite}>
      ...
      <div className="mt-6 flex flex-col gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-700 px-6 font-black text-white shadow-sm disabled:opacity-50 sm:min-w-48" disabled={busy || teacherQuery.trim().length < 3 || !courseIds.length}>{busy ? <Loader2 className="size-4 animate-spin" /> : <UserPlus className="size-4" />}Send invitation</button>
      </div>
    </form>
