  ...
  return (
    <main className="mx-auto grid w-full max-w-[96rem] gap-6">
      ...
      <SkulKidCard className="overflow-hidden">
        ...
        <AdminTableScroll className="hidden md:block"><table className={adminTable}><thead className={adminTableHead}><tr><th className={`${adminTableHeadCell} w-12`}><input aria-label="Select page" checked={Boolean(result?.users.length) && selected.length === result?.users.length} className="size-4 accent-emerald-600" onChange={(event) => setSelected(event.target.checked ? result?.users.map((user) => user.id) ?? [] : [])} type="checkbox" /></th><th className={adminTableHeadCell}>Person</th><th className={adminTableHeadCell}>Role</th><th className={adminTableHeadCell}>Status</th><th className={adminTableHeadCell}>Last activity</th><th className={`${adminTableHeadCell} w-16`}><span className="sr-only">Actions</span></th></tr></thead><tbody className={adminTableBody}>{loading ? <tr><td className="px-5 py-12 text-center font-bold text-slate-500" colSpan={6}>Loading people…</td></tr> : result?.users.map((user) => <UserRow key={user.id} user={user} checked={selected.includes(user.id)} onCheck={(checked) => setSelected((current) => checked ? [...current, user.id] : current.filter((id) => id !== user.id))} onOpen={() => setDetailId(user.id)} />)}</tbody></table></AdminTableScroll>
...
function UserRow({ user, checked, onCheck, onOpen }: { user: User; checked: boolean; onCheck: (value: boolean) => void; onOpen: () => void }) { return <tr className={adminTableRow}><td className={adminTableCell}><input aria-label={`Select ${user.displayName}`} checked={checked} className="size-4 accent-emerald-600" onChange={(event) => onCheck(event.target.checked)} type="checkbox" /></td><td className={adminTableCell}><button className="flex items-center gap-3 text-left" onClick={onOpen}><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-emerald-50 text-emerald-800 ring-1 ring-emerald-100"><UserRound className="size-5" /></span><span className="min-w-0"><b className="block truncate text-slate-950">{user.displayName}</b><span className="block max-w-xs truncate text-xs text-slate-500">{user.email || user.phone || user.id}</span></span></button></td><td className={adminTableCell}><span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-black capitalize text-slate-700 ring-1 ring-inset ring-slate-200"><Shield className="size-3" />{user.role}</span></td><td className={adminTableCell}><Status status={user.status} /></td><td className={`${adminTableCell} whitespace-nowrap text-slate-600`}>{user.lastSignInAt ? new Date(user.lastSignInAt).toLocaleString() : "Never signed in"}</td><td className={adminTableCell}><button aria-label={`Manage ${user.displayName}`} className="grid size-10 place-items-center rounded-xl border border-transparent text-slate-500 transition hover:border-slate-200 hover:bg-white hover:text-slate-900 hover:shadow-sm" onClick={onOpen}><MoreHorizontal className="size-5" /></button></td></tr>; }
