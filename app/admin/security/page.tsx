...
export default function AdminSecurityPage() {
  ...
  return <main className="mx-auto grid w-full max-w-[96rem] gap-6">
    ...
    <SkulKidCard className="overflow-hidden">
      ...
      <AdminTableScroll><table className={adminTable}><thead className={adminTableHead}><tr><th className={adminTableHeadCell}>Action</th><th className={adminTableHeadCell}>Target</th><th className={adminTableHeadCell}>Reason</th><th className={adminTableHeadCell}>Actor</th><th className={adminTableHeadCell}>Time</th><th className={adminTableHeadCell}>Result</th></tr></thead><tbody className={adminTableBody}>{loading ? <tr><td className="px-5 py-12 text-center font-bold text-slate-500" colSpan={6}>Loading audit history…</td></tr> : events.length ? events.map((event) => <tr className={adminTableRow} key={event.id}><td className={`${adminTableCell} font-black capitalize text-slate-900`}>{event.action.replaceAll(".", " ")}</td><td className={`${adminTableCell} text-slate-600`}>{event.targetType}{event.targetId ? ` · ${event.targetId.slice(0, 8)}…` : ""}</td><td className={`${adminTableCell} max-w-sm whitespace-normal leading-6 text-slate-600`}>{event.reason || "No reason recorded"}</td><td className={`${adminTableCell} font-mono text-xs text-slate-500`}>{event.actorId?.slice(0, 8) ?? "system"}</td><td className={`${adminTableCell} whitespace-nowrap text-slate-600`}>{new Date(event.createdAt).toLocaleString()}</td><td className={adminTableCell}><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-black capitalize ring-1 ring-inset ${event.result === "success" ? "bg-emerald-50 text-emerald-800 ring-emerald-200" : "bg-rose-50 text-rose-800 ring-rose-200"}`}>{event.result}</span></td></tr>) : <tr><td className="px-5 py-12 text-center font-bold text-slate-500" colSpan={6}>No audit events match this search.</td></tr>}</tbody></table></AdminTableScroll>
    </SkulKidCard>
  </main>;
}
